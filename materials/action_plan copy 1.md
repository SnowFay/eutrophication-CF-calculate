...
| 数据集 | 位置 | 主要用途 |
| --- | --- | --- |
| HydroRIVERS v1.0 Asia | `.../HydroRIVERS_v10_as.shp` | 河段几何、`DIS_AV_CMS`、`NEXT_DOWN`，用于构建河网拓扑、河段长度。|
| RiverATLAS v1.0 | `.../RiverATLAS_v10.gdb` | 提供丰富的河段属性（流量、径流、土地利用、人口等），可按 `HYRIV_ID` 与 HydroRIVERS 对应。|
| BasinATLAS v1.0 | `.../BasinATLAS_v10.gdb` | 各级 HydroBASINS 子流域属性（`dis_m3_pyr`, `run_mm_syr`, `pop_ct`, `dor_pc_pva` 等）。|
| HydroLAKES / LakeATLAS | `HydroLAKES_*`, `LakeATLAS_v10.gdb` | 湖泊/水库面、体积、均深，用于 Vtot、kret 计算。|
| HydroBASINS level 10 & pour points | `hybas_*` | 流域拓扑、Pfafstetter 码，与网格/河段映射。|
| HSWUD 水资源利用 | `data/water use data/27610524/*.nc` | 1965–2022 月尺度行业取水（dom/ele/irr/manu），支撑 `f_WTA`、`f_DITW`、水资源压力估算。|
| 参考文献/笔记 | `materials/articles/*.md` | Helmes/Henderson 原文 & 摘要，可直接引用公式与参数。|
